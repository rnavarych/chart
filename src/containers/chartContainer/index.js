...
function ChartContainer(props) {
  ...
  const dateByWeek = React.useMemo(() => dataByWeek(mockBloodGlucose(62, 3)), [
    mockBloodGlucose,
  ]);
  ...
  const [dateDescription, setDateDescription] = React.useState(
    descriptionDate(data[0].end_time, data[data.length - 1].end_time),
  );
      ...
      if (dateByWeek.length > rightIndex + 1) {
        ...
        if (nextWeek.length > 0) {
          setDateDescription(
            descriptionDate(
              data[0].end_time,
              nextWeek[nextWeek.length - 1].end_time,
            ),
          );
  ...
  const errorScroll = React.useCallback(
    (error) => {
  ...
  const showNextPage = React.useCallback(() => {
    ...
    setRightIndex(rightIndex + 1);
    setDateDescription(
      descriptionDate(date[0].end_time, date[date.length - 1].end_time),
    );
    scrollToEnd();
